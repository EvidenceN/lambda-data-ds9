...
"""
Functions for doing data analysis. Taking the functions already created and turning
then into classes. 
...
class data_split:
    ...
    def split_data(self, df):
        ...
        '''
        self.df = df
        self.train_1, self.test = train_test_split(
            self.df, test_size=0.15, random_state=42)

        self.train, self.val = train_test_split(
            self.train_1, test_size=0.20, random_state=42)

        return self.train, self.test, self.val

# function to split into target and features datasets.

class x_y_split:
    '''This class splits data into x_train, y_train, 
    x_val, y_val, x_test, y_test'''

    def __init__(self):
        pass

    def get_target(self, target_feature, train, test, val):
        '''
        function to split into target and features dataset.

        Target = target from the dataframe. 

        train = train dataset, test = test dataset, val = val dataset
